...
bool Lambertian::Scatter(const Ray& incident, const RayHit& raycastHit, color3_t& attenuation, Ray& scattered) const {
    // set scattered ray using random direction from normal, diffuse the outgoing ray
    scattered.origin = raycastHit.point; //<raycast hit point>;
    scattered.direction = glm::normalize(raycastHit.normal+random::onUnitSphere());//<raycast hit normal + random point on unit sphere>);
...
bool Metal::Scatter(const Ray& incident, const RayHit& raycastHit, color3_t& attenuation, Ray& scattered) const {
    ...
    return (glm::dot(scattered.direction, raycastHit.normal) > 0);//<dot product with scattered direction and raycast hit normal) > 0;
}
...
bool Dielectric::Scatter(const Ray& incident, const RayHit& raycastHit, color3_t& attenuation, Ray& scattered) const {
    glm::vec3 outNormal;
    float ni_over_nt;
    float cosine;
