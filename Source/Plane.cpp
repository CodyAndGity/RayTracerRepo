...
bool Plane::Hit(const Ray& ray, float minDistance, float maxDistance, RayHit& raycastHit) {
    float t;
    ...
    raycastHit.distance = t; // t
    raycastHit.point = ray.at(t);// ray at(t)
    raycastHit.normal = normal;// normal
    raycastHit.material = material.get(); // material pointer (get())
...
bool Plane::Raycast(const Ray& ray, const glm::vec3& point, const glm::vec3& normal, float minDistance, float maxDistance, float& t)
{
    ...
    float denominator = glm::dot(ray.direction, normal); // dot product between ray.direction and normal
    ...
    return t > minDistance && t < maxDistance;// t is within > min distance and < max distance
}
